...
export const stakingContractsInfo = (async () => {
  ...
  if(expectedNetwork.VUE_APP_STAKE_TYPES_AVAILABLE) {
    availableStakingContracts = expectedNetwork.VUE_APP_STAKE_TYPES_AVAILABLE
      .split(',')
      .filter(isStakeType);
  }
  ...
  if(availableStakingContracts.includes('lp')) {
    temp.lp = {
      stakingRewardsAddress: expectedNetwork.VUE_APP_LP_STAKING_REWARDS_CONTRACT_ADDRESS,
  ...
  if(availableStakingContracts.includes('lp2')) {
    temp.lp2 = {
      stakingRewardsAddress: expectedNetwork.VUE_APP_LP_2_STAKING_REWARDS_CONTRACT_ADDRESS,
...
export const stakeTypeThatCanHaveUnclaimedRewardsStakedTo = (async () => {
  const expectedNetwork = await getAddressesAuto();
  const stakeTypeForUnclaimedRewards = expectedNetwork.VUE_APP_STAKE_TYPE_FOR_UNCLAIMED_REWARDS;
  const temp: StakeType = stakeTypeForUnclaimedRewards && isStakeType(stakeTypeForUnclaimedRewards)
